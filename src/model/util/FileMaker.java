...
public class FileMaker {
	...
	public static void main(String[] args) {
		...
		listModule.addArmeLaser(0, new Arme("Laser MKI", "Petit module d'arme laser", 5, 90, 3, 45, 5, 10, setRessource(0,0,1,0,10,1,0), EnumTypeArme.LASER));
		listModule.addArmeLaser(1, new Arme("Laser MKII", "Version am�lior� du laser MKI", 5, 75, 6, 45, 5, 15, setRessource(0,0,2,0,10,1,0),EnumTypeArme.LASER));
		

		//setRessource(gaz, science, cristal, production, credit, acier, puissance)
		//new Arme(nom, description, dommage, precision, nbTire, tauxFeu, critique, point, cout)
		listModule.addArmeCinetique(0, new Arme("Type 490/3", "Des canon de la marine modifier pour l'environement spacial", 20, 60, 3, 5, 10, 20, setRessource(0,0,0,0,10,3,0),EnumTypeArme.CINETIQUE));
		listModule.addArmeCinetique(1, new Arme("Type 490/2", "Version modifier de notre pr�c�dent canon", 20, 75, 2, 5, 10, 25, setRessource(0,0,1,0,10,3,0),EnumTypeArme.CINETIQUE));
		listModule.addArmeCinetique(2, new Arme("Canon �lectrique", "Une arme � projectile acc�l�r� par une force �lectromagn�tique", 70, 85, 1, 0, 25, 70, setRessource(1,0,2,0,50,5,0),EnumTypeArme.CINETIQUE));
		

		//setRessource(gaz, science, cristal, production, credit, acier, puissance)
		//new Arme(nom, description, dommage, precision, nbTire, tauxFeu, critique, point, cout)
		listModule.addArmePlasma(0, new Arme("Fusil Plasma", "Cette arme a un fort potentielle destructeur", 60, 10, 1, 60, 50, 25, setRessource(2,0,0,0,50,1,0),EnumTypeArme.PLASMA));
		listModule.addArmePlasma(1, new Arme("Fusil Plasma jumel�", "Arme plasma jumel�", 30, 20, 2, 60, 25, 30, setRessource(3,0,0,0,60,1,0),EnumTypeArme.PLASMA));
		

		//setRessource(gaz, science, cristal, production, credit, acier, puissance)
		//new Blindage(nom, description, point, valeurBlindage, valeurBlouclier, cout)
		listModule.addBlindage(0, new Blindage("Alliage m�talique", "Blindage sommaire", 5, 80, 30, setRessource(0,0,0,0,50,2,0)));
		listModule.addBlindage(1, new Blindage("Alliage Composite", "Melange de mat�riaux diver", 10, 150, 80, setRessource(0,0,0,0,60,3,0)));
		listModule.addBlindage(2, new Blindage("Arc electrique", "Des arc electrifi� pour r�duire les chance de frape directe", 10, 0, 120, setRessource(0,0,1,0,50,2,0)));
		listModule.addBlindage(3, new Blindage("Aliage composite MKII", "Blindage renforc�", 20, 200, 110,setRessource(0,0,1,0,50,6,0)));
		
		//setRessource(gaz, science, cristal, production, credit, acier, puissance)
		//new Chassie(nom, description, point, sante, santeMax, bouclier, bouclierMax, nbArme, nbBlindage, nbModule,cout)
		listModule.addChassie(0, new Chassie("Sonde", "Parfaite pour explor� l'espace en s�curit�", 1, 1, 1, 0, 0, 0, 0, 0, setRessource(0,0,0,0,0,0,0)));
		listModule.addChassie(1, new Chassie("Classe WarHound", "Con�u pour les petit vaisseaux", 50, 150, 150, 20, 20, 2, 1, 1, setRessource(0,0,0,0,0,0,0)));
		listModule.addChassie(2, new Chassie("Classe DauntLess", "Chassie pour les vaisseaux de reconaissance", 70, 200, 200, 30, 30, 3, 1, 2, setRessource(0,0,0,0,0,0,0)));
		listModule.addChassie(3, new Chassie("Classe StarLight", "Chassie pour les vaisseaux de ligne ou d'escorte", 120, 300, 300, 50, 50, 4, 2, 2, setRessource(0,0,0,0,0,0,0)));
		listModule.addChassie(4, new Chassie("Classe NightBringer", "Chassie pour les vaisseaux de combat",350, 500, 500, 110, 110, 6, 4, 3, setRessource(0,0,0,0,0,0,0)));
		...
		listBatiment.addBatimentsVille(5, new BatimentVille("Banque central plan�taire", "Centralise les fonts de la colonie", 0, setRessource(0,0,0,-1,10,0,0), 80));		
		...
		listTech.addScienceBatiment(2, new Science("Defence plan�taire", "Delock la construction de defence plan�taire", false, 20, 0, 0));
		...
		listTech.addScienceMillitaire(5, new Science("Vaisseau d'escorte", "Délock les vaisseaux tier 2", false, 60, 1, 2));
...
//		setRessource(gaz, science, cristal, production, credit, acier)		
		try {
			...
			listVaisseau.addVaisseau(0,new Corvette("Reaper", new Chassie("Reaper", "Chassie petit et rapide", 20, 60, 60, 20, 20, 2, 1, 0, setRessource(5, 0, 5, 0, 500, 5,0)),armes, blindages, 0,0,null));
		} catch (Exception e) {
		...
		try {
			...
			blindages.put(0, listModule.getBlindage().get(0));
			listVaisseau.addVaisseau(1, new Corvette("Specter", new Chassie("Specter", "Cargo civil modifier", 5, 30, 30, 0, 0, 1, 1, 0,setRessource(2, 0, 2, 0, 200, 2, 0)),armes, blindages, 0,0,null));
		} catch (Exception e) {
			System.out.println(e.toString());
		...
		try {
			listVaisseau.addVaisseau(2, new VaisseauCivil("Sonde", 0, new Chassie("Sonde", "Créé pour l'exploration spacial", 1, 1, 1, 0, 0, 0, 0, 0, setRessource(1, 0, 1, 0, 50, 1, 0)), 0,null));
		} catch (Exception e) {
			System.out.println(e.toString());
		...
		try {
			listVaisseau.addVaisseau(3, new VaisseauCivil("Cargo colonial", 0, new Chassie("Cargo", "Cargo", 1, 80, 80, 0, 0, 0, 0, 0, setRessource(0, 0, 0, 0, 1000, 5, 0)), 0,null));
		} catch (Exception e) {
			System.out.println(e.toString());
		...
		try {
			...
			listVaisseau.addVaisseau(4, new Croiseur("Pillager", new Chassie("Pillager", "Conçue pour piller", 5, 30, 30, 0, 0, 3, 2, 0, setRessource(5, 0, 5, 0, 500, 5, 0)),armes, blindages, 0, 0, null));
		} catch (Exception e) {
			System.out.println(e.toString());
