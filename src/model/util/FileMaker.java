...
public class FileMaker {
	...
	public static void main(String[] args) {
		...
		listModule.addArmeLaser(1, new Arme("Laser MKII", "Version am�lior� du laser MKI", 5, 75, 6, 45, 5, 15, setRessource(0,0,2,0,10,1)));
		...
		listModule.addArmeCinetique(1, new Arme("Type 490/2", "Version modifier de notre pr�c�dent canon", 20, 75, 2, 5, 10, 25, setRessource(0,0,1,0,10,3)));
		listModule.addArmeCinetique(2, new Arme("Canon �lectrique", "Une arme � projectile acc�l�r� par une force �lectromagn�tique", 70, 85, 1, 0, 25, 70, setRessource(1,0,2,0,50,5)));
		...
		listModule.addArmePlasma(1, new Arme("Fusil Plasma jumel�", "Arme plasma jumel�", 30, 20, 2, 60, 25, 30, setRessource(3,0,0,0,60,1)));
		

		//setRessource(gaz, science, cristal, production, credit, acier)
		//new Blindage(nom, description, point, valeurBlindage, valeurBlouclier)
		listModule.addBlindage(0, new Blindage("Alliage m�talique", "Blindage sommaire", 5, 80, 30, setRessource(0,0,0,0,50,2)));
		listModule.addBlindage(1, new Blindage("Alliage Composite", "Melange de mat�riaux diver", 10, 150, 80, setRessource(0,0,0,0,60,3)));
		listModule.addBlindage(2, new Blindage("Arc electrique", "Des arc electrifi� pour r�duire les chance de frape directe", 10, 0, 120, setRessource(0,0,1,0,50,2)));
		listModule.addBlindage(3, new Blindage("Aliage composite MKII", "Blindage renforc�", 20, 200, 110,setRessource(0,0,1,0,50,6)));
		
		//new Chassie(nom, description, point, sante, santeMax, bouclier, bouclierMax, nbArme, nbBlindage, nbModule)
		listModule.addChassie(0, new Chassie("Sonde", "Parfaite pour explor� l'espace en s�curit�", 1, 1, 1, 0, 0, 0, 0, 0));
		listModule.addChassie(1, new Chassie("Classe WarHound", "Con�u pour les petit vaisseaux", 50, 150, 150, 20, 20, 2, 1, 1));
		...
		//new Science(nom, description, rechercher, cout, dependanceUn, dependanceDeux)
		listTech.addScienceBonus(0, new Science("Recherche des modules basic", "Le commencement des am�liorations", true, 0, 0, 0));
		...
		listTech.addScienceBonus(2, new Science("Module d'arme perforante", "Auguemente de 10% les dommages des armes cin�tique", false, 50, 0, 0));
		...
		listTech.addScienceMillitaire(4, new Science("RailGun", "Permet la construction d'un canon �l�ctrique", false, 100, 3, 0));
		listTech.addScienceMillitaire(5, new Science("Vaisseau d'escorte", "D�lock les vaisseau de tier 2", false, 60, 1, 2));
		listTech.addScienceMillitaire(6, new Science("Croiseur stailaire", "D�lock les vaisseau de tier 3", false, 150, 4, 5));
