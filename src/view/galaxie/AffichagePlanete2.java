...
public class AffichagePlanete2 extends SplitPane {
	private TextButton batiment1;
	private TextButton batiment2;
	
	public AffichagePlanete2(Planete planete, Skin skin) {
		...
		batiment1.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				if (planete.getJoueur() != null) {
					setFirstWidget(new SelectBatiment2(planete, 0, skin));
				}
			}
		...
		batiment2.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				if (planete.getJoueur() != null) {
					setSecondWidget(new SelectBatiment2(planete, 1, skin));
				}
			}
